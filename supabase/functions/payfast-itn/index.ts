...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!m_payment_id || !pf_payment_id) {
      throw new Error("Invalid ITN data - missing payment IDs");
    }
    ...
    // Create MD5 hash for verification
    ...
    if (hashBuffer) {
      ...
      if (calculatedSignature !== signature) {
        console.warn("Signature mismatch - proceeding anyway for testing");
      }
    }
    ...
    const { error: orderError } = await supabase
      ...
      .update({
        ...
        status: payment_status === "COMPLETE" ? "confirmed" : "pending",
        updated_at: new Date().toISOString(),
      })
      .eq("id", m_payment_id);
